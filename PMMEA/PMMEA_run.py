...
img_features, name_features, char_features,att_features,rel_features   = load_aux_features(file_dir=f"mmkg/{dataname}/norm")
...
pos = align_embedding(dataname,adj1 ,adj2 , adj_matrix, node_index1, node_index2, node_list1, node_list2, K_nei=7,dim=pos_dim,use_method="netmf")
...
def get_features(train_pair, pos_feature, extra_feature=None,alpha=1e-2):
    ...
    for i in range(2):
        ...
        new_rel_feature = math.sqrt(beta)*batch_sparse_matmul(rel_ent_graph, ent_feature) + batch_sparse_matmul(rel_ent_graph, pos_feature)*math.sqrt(alpha)
...
for epoch in range(epochs):
    ...
    s_features = get_features(train_pair, ent_pos.astype(np.float32),alpha=1e-5)
